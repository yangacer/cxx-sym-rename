...
static bool ParseSourceName(State *state) {
  ...
  if (ParseNumber(state, &length) &&
      ...
      std::string out(state->mangled_begin + copy.mangled_idx,
                      state->parse_state.mangled_idx - copy.mangled_idx);
      state->source_names->emplace_back(std::move(out));
    return true;
  }
  state->parse_state = copy;
  return false;
...
bool Demangle(const char* mangled, char* out, size_t out_size, std::vector<std::string>* source_names) {
  State state;
  InitState(&state, mangled, out, out_size, source_names);
